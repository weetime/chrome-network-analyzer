...
async function streamToAI(
  ...
  try {
    ...
    switch (providerKey) {
      case 'OPENAI':
        result = await AiOpenAIConnector.streamOpenAI(
          analysisData,
          apiKey,
          model,
          maxTokens,
        ...
        break;
      case 'ANTHROPIC':
        result = await AiAnthropicConnector.streamAnthropic(
          analysisData,
          apiKey,
          model,
          maxTokens,
        ...
        break;
      case 'DEEPSEEK':
        result = await AiDeepseekConnector.streamDeepseek(
          analysisData,
          apiKey,
          model,
          maxTokens,
    ...
    try {
      await AiCacheManager.cacheAnalysisResult(provider, model, analysisData, language, result);
    ...
    if (error.name === 'AbortError') {
      throw new Error(
        `AI analysis aborted: ${error.message || 'Request was cancelled or timed out'}`
      );
    }
    throw error;
  }
