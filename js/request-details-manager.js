...
function showRequestDetails(request) {
  ...
  const formatTime =
    TableManager && TableManager.formatTime ? TableManager.formatTime : time => `${time}ms`;

  const formatSize =
    TableManager && TableManager.formatSize ? TableManager.formatSize : size => `${size} bytes`;

  const getLoadTimeClass =
    TableManager && TableManager.getLoadTimeClass ? TableManager.getLoadTimeClass : () => '';
  ...
  const timingData = {
    ...
    wait:
      request.ttfb - (request.dnsTime || 0) - (request.connectTime || 0) - (request.tlsTime || 0) ||
      0,
  ...
  // 计算总时间用于百分比显示
  const totalTime =
    request.totalTime || Object.values(timingData).reduce((sum, time) => sum + time, 0) || 1; // 防止除以0
  ...
  copyDetailsBtn.innerHTML =
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"></path></svg> Copy Details';
...
function closeRequestDetails() {
  ...
  setTimeout(() => {
    ...
    const selectedRow = document.querySelector(
      '.request-table-row.selected, #requestsTableBody tr.selected-row'
    );
    if (selectedRow) {
