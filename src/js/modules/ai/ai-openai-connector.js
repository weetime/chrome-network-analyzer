...
async function sendToOpenAI(
  ...
  try {
    // 选择模型
    const modelId =
      AI_PROVIDERS.OPENAI.models[model] ||
      AI_PROVIDERS.OPENAI.models[AI_PROVIDERS.OPENAI.defaultModel];
      ...
      while (retryCount <= MAX_RETRIES) {
        try {
          ...
          if (error.name === 'AbortError' || error.name === 'DOMException') {
            ...
            lastError = new Error(
              `Connection to ${endpoint.name} failed: Request timed out or was aborted`
            );
          } else {
            lastError = error;
          }
